...
Complete guide to running Agent-SaveMark locally for development.
...
|----------|---------|-------------|
| `FDP_DATABASE__URL` | `sqlite:///./data/Agent-SaveMark.db` | Database URL (SQLite or PostgreSQL) |
...
Agent-SaveMark ships a built-in **Model Context Protocol** server at `/mcp` that MCP-capable agents (Claude Desktop, Cursor, Claude Code, Codex) can use as persistent memory. Agents authenticate with **Personal Access Tokens** (PATs) you mint from the UI.
