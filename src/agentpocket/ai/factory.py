...
from agentpocket.ai.base import NoOpChatProvider
...
def get_embedding_provider():
    ...
    if embedding_provider == "nvidia" and nvidia_key:
        try:
            from agentpocket.ai.nvidia_embeddings import NvidiaEmbeddingProvider
            return NvidiaEmbeddingProvider()
        except Exception as e:
            logger.warning("Failed to create NVIDIA embedding provider: %s", e)

    # Default to local
    from agentpocket.ai.local_embeddings import get_local_embedder
