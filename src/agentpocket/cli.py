"""Agent-SaveMark CLI — full-featured entry point with app.sh parity."""
...
APP_NAME = "Agent-SaveMark"
DEFAULT_DATA_DIR = Path.home() / ".Agent-SaveMark"
...
def _get_version():
    from agentpocket import __version__
    return __version__


def _find_env_file():
    """Find .env file. Priority: CWD > ~/.Agent-SaveMark/."""
...
def setup_wizard():
    ...
    if choice == "2":
        ...
        print()
        print("  NOTE: Install PostgreSQL support with: pip install Agent-SaveMark[postgres]")
        print()
    else:
        config["FDP_DATABASE__URL"] = f"sqlite:///{data_dir}/Agent-SaveMark.db"
    ...
    lines = [
        "# Agent-SaveMark Configuration",
        "# Generated by: Agent-SaveMark setup",
        "# Edit this file to change settings, or re-run: Agent-SaveMark setup",
        "",
    ]
    ...
    print()
    print(f"  Quick start:       {_C.YELLOW}Agent-SaveMark start{_C.NC}")
    print(f"  With PostgreSQL:   {_C.YELLOW}Agent-SaveMark start --postgres{_C.NC}")
    print(f"  Full stack:        {_C.YELLOW}Agent-SaveMark start --full{_C.NC}")
    print()
    return env_path
...
def cmd_start(args):
    """Start the Agent-SaveMark server."""
    env_path = _load_env()
    if env_path is None:
    ...
    if profile == "sqlite" or args.sqlite:
        data_dir = DEFAULT_DATA_DIR / "data"
        data_dir.mkdir(parents=True, exist_ok=True)
        os.environ["FDP_DATABASE__URL"] = f"sqlite:///{data_dir}/Agent-SaveMark.db"
    ...
    if args.background:
        ...
        _start_process("server", [
            sys.executable, "-m", "uvicorn", "agentpocket.main:app",
        ...
        print()
        print(f"  {_C.DIM}Logs:    Agent-SaveMark logs{_C.NC}")
        print(f"  {_C.DIM}Status:  Agent-SaveMark status{_C.NC}")
        print(f"  {_C.DIM}Stop:    Agent-SaveMark stop{_C.NC}")
    else:
...
def cmd_status(_args):
    ...
    env_path = _find_env_file()
    db_url = os.environ.get("FDP_DATABASE__URL", "sqlite:///./data/Agent-SaveMark.db")
...
def cmd_db(args):
    ...
    if args.db_command == "init":
        ...
        from agentpocket.db.session import init_db
        init_db()
        ...
        if db_url.startswith("postgresql"):
            ...
            if not Path(db_path).exists():
                ...
                _warn("Run 'Agent-SaveMark db init' first.")
                return
            _info("Opening SQLite shell...")
            os.execvp("sqlite3", ["sqlite3", db_path])

    else:
        print("Usage: Agent-SaveMark db <init|reset|migrate|shell>")
...
def cmd_services(args):
    ...
    if action == "up":
        ...
        print("Usage: Agent-SaveMark services <up|down|status> [service...]")
...
{_C.CYAN}USAGE:{_C.NC}
    Agent-SaveMark <command> [options]

{_C.CYAN}GETTING STARTED:{_C.NC}
    Agent-SaveMark setup                         Interactive setup wizard
    Agent-SaveMark start                         Start server (runs setup if first time)
    Agent-SaveMark start --sqlite                Zero-config start (SQLite, no Docker)
...
{_C.CYAN}EXAMPLES:{_C.NC}
    Agent-SaveMark setup                          # First-time config wizard
    Agent-SaveMark start                          # Start (auto-setup on first run)
    Agent-SaveMark start --sqlite                 # Quick start, no Docker
    Agent-SaveMark start --postgres -d            # Background with PostgreSQL
    Agent-SaveMark start --full                   # Full stack with all services
    Agent-SaveMark stop                           # Stop server
    Agent-SaveMark status                         # Check what's running
    Agent-SaveMark db reset                       # Reset database
    Agent-SaveMark services up postgres meili     # Start specific Docker services
...
def main():
    ...
    parser.add_argument("-V", "--version", action="version", version=f"Agent-SaveMark {_get_version()}")
    ...
    if args.command == "version":
        print(f"Agent-SaveMark {_get_version()}")
