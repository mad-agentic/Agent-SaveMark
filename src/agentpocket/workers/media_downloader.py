...
from agentpocket.workers import huey
...
def download_media(item_id: str, user_id: str, media_urls: list[dict]) -> dict:
    ...
    from agentpocket.db.session import get_engine
        ...
        try:
            ...
            for _redirect in range(5):
                ...
                resp = httpx.head(
                    ...
                    headers={"User-Agent": "Agent-SaveMark/0.1"},
                )
                if resp.is_redirect:
            ...
            with httpx.stream("GET", final_url, timeout=30.0, follow_redirects=False, headers={"User-Agent": "Agent-SaveMark/0.1"}) as response:
    ...
    try:
        ...
        for _redirect in range(5):
            if not _is_safe_media_url(current_url):
                ...
                return None
            resp = httpx.head(current_url, timeout=10.0, follow_redirects=False, headers={"User-Agent": "Agent-SaveMark/0.1"})
            if resp.is_redirect:
                current_url = resp.headers.get("location", "")
                if not current_url:
                    return None
