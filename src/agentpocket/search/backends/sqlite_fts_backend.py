...
from agentpocket.search.base import KeywordHit, SearchFilters


class SqliteFtsBackend:
    def init(self, db: Session) -> None:
        from agentpocket.search.sqlite_fts import init_chunks_fts, init_fts, init_notes_fts
    ...
    def index_chunks(
        ...
        from agentpocket.search.sqlite_fts import index_chunks

        index_chunks(db, item_id, user_id, chunks, title, url)

    def delete_item(self, db: Session, item_id: uuid.UUID) -> None:
        from agentpocket.search.sqlite_fts import delete_chunks, delete_item
    ...
    def search(
        ...
        from agentpocket.search.sqlite_fts import search, search_chunks
