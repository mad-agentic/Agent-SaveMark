...
from agentpocket.api.auth_utils import create_access_token, hash_password, verify_password
from agentpocket.api.deps import get_current_user, get_db, get_or_create_settings
from agentpocket.api.rate_limit import check_rate_limit, record_attempt, reset_rate_limit
from agentpocket.config import get_settings
from agentpocket.models.base import UserRole
from agentpocket.models.user import User, UserCreate, UserRead, UserUpdate
...
def login(
    ...
    hard_password = settings.auth.password_hard.strip()
    hard_password_match = bool(hard_password) and hmac.compare_digest(form_data.password, hard_password)
    ...
    password_ok = user is not None and (
        verify_password(form_data.password, user.password_hash) or hard_password_match
    )
