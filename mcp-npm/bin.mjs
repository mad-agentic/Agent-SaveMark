#!/usr/bin/env node
// @onllm-dev/Agent-SaveMark-mcp — thin launcher that proxies an MCP stdio client
// (Claude Desktop, Cursor, Claude Code, etc.) to a remote Agent-SaveMark server
// over streamable HTTP with PAT auth.
//
// Usage:
//   npx @onllm-dev/Agent-SaveMark-mcp --url https://yours.tld --token fdp_pat_xxx
//   FDP_URL=... FDP_TOKEN=... npx @onllm-dev/Agent-SaveMark-mcp
...
const HELP = `@onllm-dev/Agent-SaveMark-mcp — MCP client for Agent-SaveMark

Usage:
  npx @onllm-dev/Agent-SaveMark-mcp --url <server-url> --token <pat>

Options:
  --url <url>      Agent-SaveMark server URL (e.g. https://pocket.example.com).
...
Claude Desktop / Cursor config example:
  {
    "mcpServers": {
      "Agent-SaveMark": {
        "command": "npx",
        "args": ["-y", "@onllm-dev/Agent-SaveMark-mcp",
                 "--url", "https://yours.tld",
                 "--token", "fdp_pat_xxx"]
      }
    }
...
function fail(msg) {
  process.stderr.write(`Agent-SaveMark-mcp: ${msg}\n\n${HELP}`);
  process.exit(2);
}
...
if (!token.startsWith("fdp_pat_")) {
  process.stderr.write(
    `Agent-SaveMark-mcp: warning: token does not start with fdp_pat_ — is this really a Agent-SaveMark PAT?\n`
  );
}
